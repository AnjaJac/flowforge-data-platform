...
def _write_analytics_parquets(analytics_dir):
    ...
    pl.DataFrame(
        {"month": ["2024-01"], "gmv": [1000.0], "aov": [100.0], "order_count": [10]}
    ...
    ).write_parquet(analytics_dir / "seller_performance.parquet")
    pl.DataFrame(
        {
            "product_id": ["p1"],
            "revenue": [600.0],
    ...
    ).write_parquet(analytics_dir / "product_performance.parquet")
    pl.DataFrame(
        {
...
def test_generate_quality_report_overall_status_passed_when_validation_passed(tmp_path):
    ...
    generate_quality_report(
        analytics_dir=str(analytics_dir), validation_result={"passed": True}
    )
...
def test_generate_quality_report_overall_status_failed_when_validation_not_passed(
    tmp_path,
):
    analytics_dir = tmp_path / "analytics"
    analytics_dir.mkdir()
    ...
    generate_quality_report(
        analytics_dir=str(analytics_dir), validation_result={"passed": False}
    )
